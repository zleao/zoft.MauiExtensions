...
public abstract partial class CoreViewModel : ObservableObject, IDisposable
{
            ...
            if (dependsOnAttributes?.Length > 0)
            {
                lock (_propertyDependencies)
                {
                    foreach (var attribute in dependsOnAttributes)
                    {
                        ...
                        if (!_propertyDependencies.TryGetValue(attribute.Name, out IList<DependencyInfo> value))
                        {
                            value = new List<DependencyInfo>();
                            _propertyDependencies.Add(attribute.Name, value);
                        }

                        value.Add(new DependencyInfo(property, attribute.IsConditional));
        ...
        foreach (var method in type.GetTypeInfo().DeclaredMethods.Where(m => m.GetParameters().Length == 0))
        {
            ...
            if (dependsOnAttributes?.Length > 0)
            {
                foreach (var attribute in dependsOnAttributes)
                {
                    ...
                    {
                        if (!_methodDependencies.TryGetValue(attribute.Name, out IList<MethodInfo> value))
                        {
                            value = new List<MethodInfo>();
                            _methodDependencies.Add(attribute.Name, value);
                        }
    ...
    public void RaiseDependenciesPropertyChanged(string dependencyName)
    {
        ...
        if (_dependsOnConditionalCount.TryGetValue(dependencyName, out int value) && value > 0)
        {
            return;
        }
    ...
    protected void ExecuteWithoutConditionalDependsOn(string propertyName, Action action)
    {
        if (MainThreadService != null)
        {
            MainThreadService.BeginInvokeOnMainThread(() =>
            {
                try
                {
                    if (!_dependsOnConditionalCount.TryGetValue(propertyName, out int value))
                        _dependsOnConditionalCount.Add(propertyName, 1);
                    else
                        _dependsOnConditionalCount[propertyName] = ++value;
